...
function propHighway(g, R, z, solid) {
  ...
  const B = (x, y, zz, sx, sy, sz, c, e = 0, mat = 16) => g.box(M4.trs(PM.a, x, y, zz, 0, 0, 0, sx, sy, sz), c, e, mat);
  B(0, 7.0, z, 136, 1.0, 14, conc);                                        // deck
  for (const o of [-5, 0, 5]) B(0, 6.2, z + o, 136, 0.7, 0.9, dark);       // girders underneath
  for (const s of [-1, 1]) { B(0, 8.0, z + s * 6.8, 136, 1.0, 0.4, conc); B(0, 8.55, z + s * 6.8, 136, 0.1, 0.45, [0.9, 0.55, 0.1], 0.25, 0); }
  for (let x = -60; x <= 60.1; x += 16) {                                   // lamp posts on the deck, most of them dead
    B(x, 10, z - 6.4, 0.18, 4, 0.18, dark, 0, 4); B(x, 11.9, z - 5.9, 0.3, 0.12, 1.1, dark, 0, 4);
    if (R() < 0.3) { B(x, 11.8, z - 5.5, 0.25, 0.05, 0.5, [1, 0.6, 0.25], 3, 0); WORLD.halos.push({ p: [x, 11.7, z - 5.5], s: 2.5, c: [0.5, 0.25, 0.08] }); }
  }
  const neon = [[1, 0.2, 0.6], [0.2, 0.9, 1], [0.9, 1, 0.2], [1, 0.4, 0.1]];
  for (let x = -60; x <= 60.1; x += 12) {                                   // pillars: pairs under a cap beam, tagged with glowing graffiti
    if (Math.abs(x) < 7) continue;
    B(x, 6.0, z, 1.6, 0.6, 11, dark);
    for (const s of [-1, 1]) {
      B(x, 3.0, z + s * 4, 1.3, 6, 1.3, conc); solid(x - 0.65, x + 0.65, 0, 6, z + s * 4 - 0.65, z + s * 4 + 0.65);
      if (R() < 0.5) B(x, 1.4 + R(), z + s * 4 - s * 0.66, 1.0, 0.6, 0.02, neon[Math.floor(R() * 4)], 0.5, 0);
    }
  }
  // flanks: jersey barriers topped with chain-link, blocking everything but the avenue
  for (const s of [-1, 1]) {
    for (let x = 10.6; x < 64; x += 3.2) propBarrier(g, s * x, z, false);
    B(s * 36.5, 2.2, z, 53, 2.4, 0.04, [0.18, 0.19, 0.2], 0, 4);
    for (let x = 9; x <= 64; x += 3.2) B(s * x, 2.2, z, 0.08, 2.4, 0.08, dark, 0, 4);
    solid(s > 0 ? 9 : -64, s > 0 ? 64 : -9, 0, 3.4, z - 0.4, z + 0.4);
  }
  // the barricade across the avenue leaves a gap to squeeze through
  for (const x of [-7.6, -4.4, 4.4, 7.6]) propBarrier(g, x, z + 0.5, false);
  propHoverCar(g, R, -5.5, z + 4.2, 1.3, [0.25, 0.08, 0.06], 0); solid(-8, -3, 0, 1.6, z + 2.8, z + 5.6);
  // sodium lamps bolted under the deck
  for (const o of [-3.5, 3.5]) { B(0, 5.75, z + o, 0.6, 0.12, 0.3, [1, 0.55, 0.2], 3, 0); WORLD.lights.push({ p: [0, 5.2, z + o], r: 11, c: [1.6, 0.75, 0.22], shop: true }); }
}
/* ---------- gas station: canopy over two pump islands and a kiosk ---------- */
function propGasStation(g, x, z, solid) {
  const dark = [0.1, 0.1, 0.11], wht = [0.55, 0.55, 0.53];
  const B = (bx, by, bz, sx, sy, sz, c, e = 0, mat = 16) => g.box(M4.trs(PM.a, x + bx, by, z + bz, 0, 0, 0, sx, sy, sz), c, e, mat);
  B(0, 0.03, 0, 16, 0.06, 12, [0.2, 0.2, 0.2]);
  B(0, 4.6, 0, 11, 0.6, 7, wht, 0, 15); B(0, 4.25, 0, 11.1, 0.12, 7.1, [0.1, 0.8, 0.5], 0.8, 0);
  for (const bx of [-4.5, 4.5]) for (const bz of [-2.8, 2.8]) { B(bx, 2.15, bz, 0.35, 4.3, 0.35, wht, 0, 4); solid(x + bx - 0.2, x + bx + 0.2, 0, 4.3, z + bz - 0.2, z + bz + 0.2); }
  for (const bx of [-2, 2]) { B(bx, 0.15, 0, 1.2, 0.3, 4, [0.25, 0.25, 0.25]); for (const bz of [-1, 1]) { B(bx, 0.95, bz, 0.6, 1.3, 0.5, dark, 0, 15); B(bx, 1.3, bz - 0.26, 0.4, 0.3, 0.02, [0.2, 1, 0.6], 0.6, 0); } solid(x + bx - 0.6, x + bx + 0.6, 0, 1.6, z - 2, z + 2); }
  B(0, 1.6, 5.2, 7, 3.2, 3, [0.35, 0.33, 0.3]); B(0, 1.5, 3.68, 5, 1.6, 0.04, [1, 0.8, 0.5], 0.5, 0); solid(x - 3.5, x + 3.5, 0, 3.2, z + 3.7, z + 6.7);
  B(-7, 4, -5, 0.3, 8, 0.3, dark, 0, 4); B(-7, 8.4, -5, 2.4, 1.4, 0.3, [0.1, 0.8, 0.5], 1.2, 0);
  WORLD.lights.push({ p: [x, 3.8, z], r: 12, c: [1.1, 1.4, 1.2], shop: true });
}
/* ---------- arched wooden footbridge over the stream ---------- */
function propFootBridge(g, x, z, solid) {
  const wood = [0.2, 0.12, 0.07], lac = [0.45, 0.07, 0.05];
  const H = [0.14, 0.28, 0.38, 0.28, 0.14];
  H.forEach((h, i) => { const zz = z - 2.5 + i + 0.5; g.box(M4.trs(PM.a, x, h / 2, zz, 0, 0, 0, 3.6, h, 1.02), wood, 0, 13); solid(x - 1.8, x + 1.8, 0, h, zz - 0.5, zz + 0.5); });
  for (const s of [-1, 1]) {
